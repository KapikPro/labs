...
void out(double* x1, double* x2,int n, int* it1,int* it2)
{
	...
	while (c < n)
	{
		...
		//с варьирующимся кол-вом знаков я так и не разобрался
		//если это не критично,то я лучше уточню у вас на паре в четверг как это делается
		c++;
	}
}
int main()
{
	...
	int n;
	cout << "введите а,b и n\n";
	cin >> a >> b >> n;
	while (true)
		if ((n < 1 || n>10) || (a >= b || a < 0 || a>2 || b < 0 || b>2))
		{
			cout << "введены неправельные входные данные, введите их ещё раз\n";
			cin >> a >> b >> n;
		}
		else
			break;
	if (check(a, b, f1) == 0)
		cout << "у фукции f1 корня на данном отрезке нет\n";
	else
	{
	...
	cout << "\n\n";
	if (check(a, b, f2) == 0)
		cout << "у фукции f2 корня на данном отрезке нет\n";
	else
	{
